...
-  $tf$ is the term frequency: how many times a term $t$ appears in the given document $d$ divided by the total number of terms in $d$. 
- and $idf$ is the inverse document frequency: a term inversely proportional to the number of documents in the corpus $D$ in which the term $t$ appears divided by the total number of documents in the corpus. Commonly the logarithm of this term is taken (to try to smooth the resulting values and also prevent zero-division errors) and we have $\log{(|D|/|\{d \in D: t \in d \}|)}$